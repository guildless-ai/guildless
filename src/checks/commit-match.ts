...
export async function checkCommitMatch(cwd: string, testedCommit: string): Promise<CheckResult> {
  try {
    ...
    return tested === head
      ...
      : { id: "commit-match", ok: false, summary: "Tested commit differs from current HEAD", detail: `tested ${tested}; HEAD ${head}` };
  } catch (error) {
    return { id: "commit-match", ok: false, summary: "Tested commit could not be resolved", detail: String(error) };
  }
}
