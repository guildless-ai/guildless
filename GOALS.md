...
## 訂正2（2026-09-13 00:50 UTC）— 会話品質を最優先に切替

- 所有者指摘: 「1時間ごとに何をチェックしているのか。人間相手に遅延しない会話品質の方が重要」。事実: 1時間ごとの確認は GitHub PR #6/#7 の状態確認のみで、7回連続変化なし。この定期確認は停止した。
- 検証した事実（apotrail DB `call_logs`）: LP 折り返しデモ（`gpt-live-1`）が、人間が話している最中でも「お声が遠いようなのですが」を繰り返す。09-12 09:04 UTC の通話（所有者自身のテスト）では user「はい、聞こえますが」「会話にならないんですけど」の後も同じ定型文を3回再生し切断された。定型文の開始時刻が user 発話の途中（14.8s）にあるため、user 音声ではなくタイマーで発火している。user 発話は transcript に記録されている＝音声認識は届いているが、応答生成/無応答監視に反映されていない。
...
## 実回線テスト（2026-09-14 08:54 / 08:58 JST）

所有者指示「実回線で試してから報告しろ」に従い、本番 LP の折り返しデモ（`POST https://apotrail.jp/api/public/test-call`）で所有者番号へ2回発信。この環境から apotrail.jp へ直接出られないため、Supabase Edge Function `guildless-apotrail-probe`（incagent-os、MONITOR_SECRET 認証、宛先は APOTRAIL_BASE_URL に固定）経由で発信した。

**結果: 2回とも留守番電話。人間との会話は検証できていない。** ただし本番の欠陥が実測で2件確定した。

| call_logs id | 通話時間 | 課金 | 内容 |
|---|---|---|---|
| 582b5712-5dbb-4a78-9a0c-caf31b5ae4d7 | 45秒 | $0.176 | 留守電応答→AI が挨拶2文（17.8秒で終了）→**27秒無言**→切断 |
| 536e1034-4900-4a8e-a9e4-8f63464ff612 | 155秒 | $0.607 | 同上（18.6秒で終了）→**136秒無言**→切断 |

- 欠陥1: 留守番電話のガイダンス（「発信先は現在電話に出ることができません」）を文字起こしできているのに、AI は機械に向かって営業の挨拶を読み上げ、その後も切らずに回線を保持する。1件で最大 $0.607。
- 欠陥2: 結果が `other`、要約が「相手が切断（会話あり: 3発話）」。3発話の内訳は留守電ガイダンス1＋AI発話2。機械の音声を人間の発話として数え、voicemail→other に補正しているため、データ上は「会話が成立した」ことになっている。
- 確認: 「お声が遠いようなのですが」の反復は発生しなかった。`twilio_call_sid`/`provider` が null のままなので **PR #6 は本番に入っていない**。現在の本番 master は LP のタイマーを無効化してあるだけで、無音時の声かけも終話もない。
- 対処: 上記2件の修正（留守電即切り・無言放置の禁止・機械音声を発話に数えない）を修正セッションへ指示。マージとデプロイは行わせていない。
- ルール追加: **架電機能の変更は「実回線1本」で確認してから完了とする。テスト通話のコストは1本あたり $0.2〜0.6 を上限の目安とし、無言のまま回線を保持しない。**

## 仮想対応テスト（AI 対 AI、実回線、2026-09-14 09:13 JST）

所有者が電話に出られないため、人間役を自社保有の AI 受電番号 `+815017855064`（「相談窓口」、tenant 36912fbd 合同会社ココアル導入テスト）に担当させ、LP 折り返しデモから実回線で発信。両側が記録された。

- 発信側 `call_logs` 155d16f6-b3e9-4405-9165-0d1b031fba24（gpt-live-1、21秒、$0.0839）
- 着信側 `call_logs` 96a5172a-03bd-451d-9b20-e5654b2035cf（gpt-realtime、22秒、$0.0274）

実測した会話（ミリ秒は発信側の記録）:

| 時刻 | 話者 | 内容 |
|---|---|---|
| 2.8–4.2秒 | AI | アポトレールの田中です。 |
| 4.8–6.4秒 | 相手 | はい、こちら相談窓口です |
| 6.2–8.8秒 | AI | LPのお申し込みでお電話しました。 |
| 15.6–23.0秒 | 相手 | かしこまりました、後ほど担当者より折り返しさせていただきます… |
| 22.8–23.0秒 | AI | はい、 |

**欠陥（すべてタイムスタンプから再現可能）**
1. かぶせ発声。相手が 6.4 秒まで話している最中の 6.2 秒に AI が話し始めた。終了間際も相手が 23.0 秒まで話している最中の 22.8 秒に発声。
2. 無言 6.8 秒。AI は 8.8 秒に2文目を終えた後、相手が話し出す 15.6 秒まで沈黙。相手側が沈黙を埋めて終話した。
3. 内容に応答しない。AI の発話は台本2文と「はい、」の3つだけ。「こちら相談窓口です」にも「後ほど折り返します」にも中身のある返答をしていない。ただし 09-12 12:02 と 09-13 08:53 の通話では質問に正しく答えているので、能力はあるが安定して発火しない。
4. 受付のあしらい「後ほど担当者より折り返します」が `result=callback` として保存され、リードとして数えられている。
5. 発信側の `twilio_call_sid`/`provider` が null、`ai_phase_seconds` が 0。

**ルール追加**: 架電機能の検証は所有者の在席に依存しない。自社の AI 受電番号を相手役にした AI 対 AI の実回線テストを回帰テストとして用意し、タイムスタンプで「かぶせなし・無言N秒以内・実質的な返答1回以上・分類が正しい」を機械判定する。実発信はフラグで明示的に有効化したときだけ行う。

学習記録: `guildless_money_cases` の `apotrail-lp-ai-vs-ai-live-test-2026-09-14`。
